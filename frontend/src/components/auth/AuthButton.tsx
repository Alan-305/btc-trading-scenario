...
export function AuthButton({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="hidden max-w-[200px] truncate text-xs text-content-secondary sm:inline" title={label}>
        {label}
      </span>
      <button
        type="button"
        onClick={onSignOut}
        className="min-h-[44px] rounded-lg border border-surface-border px-4 py-2 text-sm text-slate-300 transition hover:border-content-muted hover:bg-surface-hover"
      >
        ログアウト
      </button>
    </div>
